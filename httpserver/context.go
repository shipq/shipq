...
// CookieWriter wraps an http.ResponseWriter so that queued Set-Cookie
// headers are applied before the first call to WriteHeader or Write.
// This ensures cookies set via SetCookie are included in the response
// even when the handler writes the body before wrapHandler regains
// control.
...
// NewCookieWriter returns a CookieWriter that will inject the cookies
// collected in ops into w's headers before the response is committed.
func NewCookieWriter(w http.ResponseWriter, ops *[]CookieOp) *CookieWriter {
	return &CookieWriter{ResponseWriter: w, cookieOps: ops}
}

// Flush applies any pending Set-Cookie headers that have not yet been
// written. It is safe to call multiple times; only the first call has
// an effect. Generated code calls Flush after the handler returns to
// cover handlers that never write a response.
func (cw *CookieWriter) Flush() {
	if cw.flushed {
		return
	}
	cw.flushed = true
	for _, op := range *cw.cookieOps {
		http.SetCookie(cw.ResponseWriter, op.Cookie)
	}
}

// WriteHeader applies pending cookies and then writes the status code.
func (cw *CookieWriter) WriteHeader(code int) {
	cw.Flush()
	cw.ResponseWriter.WriteHeader(code)
}

// Write applies pending cookies (implicitly committing a 200 status)
// and then writes the body bytes.
