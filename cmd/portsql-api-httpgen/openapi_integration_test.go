...
`
	if err := os.WriteFile(filepath.Join(tmpDir, "portsql-api-httpgen.ini"), []byte(configContent), 0644); err != nil {
...
func TestOpenAPI_DeterministicOutput(t *testing.T) {
	...
	bytes1, err := BuildOpenAPI(cfg, manifest1)
	if err != nil {
		t.Fatalf("first BuildOpenAPI failed: %v", err)
	}

	bytes2, err := BuildOpenAPI(cfg, manifest2)
	if err != nil {
		t.Fatalf("second BuildOpenAPI failed: %v", err)
	}

	if string(bytes1) != string(bytes2) {
		t.Error("OpenAPI output is not deterministic across discovery runs")
	}
}
