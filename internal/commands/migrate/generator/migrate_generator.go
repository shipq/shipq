...
type MigrationConfig struct {
	...
	Columns       []parser.ColumnSpec // columns to generate
...
func GenerateMigration(cfg MigrationConfig) ([]byte, error) {
	...
	if cfg.ScopeColumn != "" && !cfg.IsGlobal {
		...
		scopeCol := parser.ColumnSpec{
			Name:       cfg.ScopeColumn,
			Type:       "references",
			References: cfg.ScopeTable,
		}
		columns = append([]parser.ColumnSpec{scopeCol}, columns...)
	}
	...
	// Write imports (use embedded lib path, not github.com/shipq/shipq)
	buf.WriteString("import (\n")
	buf.WriteString(fmt.Sprintf("\t%q\n", cfg.ModulePath+"/shipq/lib/db/portsql/ddl"))
	buf.WriteString(fmt.Sprintf("\t%q\n", cfg.ModulePath+"/shipq/lib/db/portsql/migrate"))
	buf.WriteString(")\n\n")
	...
	// Write AddTable call
	// Use = instead of := if err is already declared from reference lookups above
	if len(referencedTables) > 0 {
...
func collectReferencedTables(columns []parser.ColumnSpec) []string {
...
func generateColumnCode(col parser.ColumnSpec) string {
