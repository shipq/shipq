...
`,
		...
		{
			name: "builder pattern with Auth",
			content: `package books
...
`,
			expectedCalls: []RegisterCall{
				{Method: "Post", Path: "/books", FuncName: "CreateBook", RequireAuth: true},
				{Method: "Get", Path: "/books", FuncName: "ListBooks", RequireAuth: true},
				{Method: "Get", Path: "/books/:id", FuncName: "GetBook", RequireAuth: true},
				{Method: "Patch", Path: "/books/:id", FuncName: "UpdateBook", RequireAuth: true},
				{Method: "Delete", Path: "/books/:id", FuncName: "SoftDeleteBook", RequireAuth: true},
			},
			expectError: false,
		},
		{
			name: "mixed auth and non-auth builder pattern",
			content: `package pets
...
`,
			expectedCalls: []RegisterCall{
				{Method: "Post", Path: "/pets", FuncName: "CreatePet", RequireAuth: true},
				{Method: "Get", Path: "/pets", FuncName: "ListPets", RequireAuth: false},
				{Method: "Get", Path: "/pets/:id", FuncName: "GetPet", RequireAuth: false},
				{Method: "Patch", Path: "/pets/:id", FuncName: "UpdatePet", RequireAuth: true},
				{Method: "Delete", Path: "/pets/:id", FuncName: "SoftDeletePet", RequireAuth: true},
			},
			expectError: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, expected := range tt.expectedCalls {
				...
				if actual.RequireAuth != expected.RequireAuth {
					t.Errorf("call %d: expected requireAuth %v, got %v", i, expected.RequireAuth, actual.RequireAuth)
				}
				if actual.Line == 0 {
