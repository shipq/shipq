...
func collectRunnerImports(cfg UnifiedRunnerConfig, queries []userQueryInfo) map[string]bool {
	...
	if cfg.Schema != nil {
		for _, table := range cfg.Schema.Schema.Tables {
			...
			if analysis.HasCreatedAt || analysis.HasUpdatedAt || analysis.HasDeletedAt {
				imports["time"] = true
			}
...
func writeGetTypes(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, pascal string) {
	// Params
	buf.WriteString(fmt.Sprintf("// Get%sParams are the parameters for fetching a single %s.\n", pascal, singular(tableName)))
	buf.WriteString(fmt.Sprintf("type Get%sParams struct {\n", pascal))
	if analysis.HasPublicID {
		buf.WriteString("\tPublicID string\n")
	} else if analysis.PrimaryKey != nil {
		mapping := codegen.MapColumnType(*analysis.PrimaryKey)
		buf.WriteString(fmt.Sprintf("\t%s %s\n", dbstrings.ToPascalCase(analysis.PrimaryKey.Name), mapping.GoType))
	}
	buf.WriteString("}\n\n")

	// Result
	buf.WriteString(fmt.Sprintf("// Get%sResult is the result of fetching a single %s.\n", pascal, singular(tableName)))
...
func writeListTypes(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis) {
	...
	// Params (empty for now, could add pagination later)
	...
	buf.WriteString("\t// Add pagination parameters as needed\n")
	buf.WriteString("}\n\n")

	// Result item
	singular := dbstrings.ToSingular(tableName)
	pascalSingular := dbstrings.ToPascalCase(singular)
	...
	buf.WriteString("}\n\n")
	_ = pascalSingular // suppress unused
}
...
func writeUpdateTypes(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, pascal string) {
	// Params
	buf.WriteString(fmt.Sprintf("// Update%sParams are the parameters for updating a %s.\n", pascal, singular(tableName)))
...
func writeDeleteTypes(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, pascal string) {
	// Params
	buf.WriteString(fmt.Sprintf("// Delete%sParams are the parameters for deleting a %s.\n", pascal, singular(tableName)))
...
func writeGetMethod(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, pascal string, cfg UnifiedRunnerConfig) {
	typesPackage := "queries"
	paramType := fmt.Sprintf("%s.Get%sParams", typesPackage, pascal)
	resultType := fmt.Sprintf("%s.Get%sResult", typesPackage, pascal)
	...
	buf.WriteString("\targs := []any{")
	if analysis.HasPublicID {
		buf.WriteString("params.PublicID")
	} else if analysis.PrimaryKey != nil {
		buf.WriteString(fmt.Sprintf("params.%s", dbstrings.ToPascalCase(analysis.PrimaryKey.Name)))
	}
	buf.WriteString("}\n")
...
func writeListMethod(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, cfg UnifiedRunnerConfig) {
	typesPackage := "queries"
	plural := dbstrings.ToPascalCase(tableName)
	paramType := fmt.Sprintf("%s.List%sParams", typesPackage, plural)
	resultType := fmt.Sprintf("%s.List%sItem", typesPackage, plural)

	buf.WriteString(fmt.Sprintf("// List%s fetches all %s.\n", plural, tableName))
	buf.WriteString(fmt.Sprintf("func (r *QueryRunner) List%s(ctx context.Context, params %s) ([]%s, error) {\n", plural, paramType, resultType))
	buf.WriteString(fmt.Sprintf("\t_ = params // Params reserved for future pagination\n"))
...
func writeUpdateMethod(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, pascal string, cfg UnifiedRunnerConfig) {
	typesPackage := "queries"
	paramType := fmt.Sprintf("%s.Update%sParams", typesPackage, pascal)

	buf.WriteString(fmt.Sprintf("// Update%s updates an existing %s.\n", pascal, singular(tableName)))
	buf.WriteString(fmt.Sprintf("func (r *QueryRunner) Update%s(ctx context.Context, params %s) (sql.Result, error) {\n", pascal, paramType))

	// Build args - SET values first, then WHERE condition
	buf.WriteString("\targs := []any{\n")
	for _, col := range table.Columns {
		if col.Name == "id" || col.Name == "public_id" || col.Name == "created_at" || col.Name == "updated_at" || col.Name == "deleted_at" {
			continue
	...
	if analysis.HasPublicID {
		...
		buf.WriteString(fmt.Sprintf("\t\tparams.%s,\n", dbstrings.ToPascalCase(analysis.PrimaryKey.Name)))
	}
	buf.WriteString("\t}\n")

	buf.WriteString(fmt.Sprintf("\treturn r.db.ExecContext(ctx, r.update%sSQL, args...)\n", pascal))
	buf.WriteString("}\n\n")
}
...
func writeDeleteMethod(buf *bytes.Buffer, tableName string, table ddl.Table, analysis codegen.TableAnalysis, pascal string, cfg UnifiedRunnerConfig) {
	typesPackage := "queries"
	paramType := fmt.Sprintf("%s.Delete%sParams", typesPackage, pascal)

	buf.WriteString(fmt.Sprintf("// Delete%s deletes a %s", pascal, singular(tableName)))
	if analysis.HasDeletedAt {
		buf.WriteString(" (soft delete).\n")
	} else {
		buf.WriteString(".\n")
	}
	buf.WriteString(fmt.Sprintf("func (r *QueryRunner) Delete%s(ctx context.Context, params %s) (sql.Result, error) {\n", pascal, paramType))
	...
	buf.WriteString(fmt.Sprintf("\treturn r.db.ExecContext(ctx, r.delete%sSQL, args...)\n", pascal))
	buf.WriteString("}\n\n")
}
