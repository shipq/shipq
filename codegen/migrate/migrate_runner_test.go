...
func TestGenerateMigrateRunner(t *testing.T) {
	t.Run("generates valid go code", func(t *testing.T) {
		...
		if !strings.Contains(contentStr, `"example.com/myapp/shipq/lib/db/portsql/migrate"`) {
			t.Error("generated code missing embedded migrate package import")
		}
		if strings.Contains(contentStr, `"github.com/shipq/shipq/`) {
