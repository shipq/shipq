...
func generateTestHarnessImports(buf *bytes.Buffer, cfg HTTPTestHarnessGenConfig) {
	...
	// Import dialect-specific runner package
	fmt.Fprintf(buf, "\tdbrunner %q\n", cfg.ModulePath+"/shipq/queries/"+cfg.DBDialect)
	buf.WriteString(")\n\n")
}

// generateTestServerStruct generates the TestServer struct definition.
func generateTestServerStruct(buf *bytes.Buffer) {
	buf.WriteString(`// txWrapper wraps *sql.Tx to implement PingableQuerier for test use.
// NewMux requires PingableQuerier (Querier + Ping), but *sql.Tx only implements Querier.
// The Ping() method is a no-op since we're in a transaction that's already valid.
type txWrapper struct {
	*sql.Tx
}

func (w *txWrapper) Ping() error {
	return nil
}

// TestServer wraps httptest.Server with transaction management.
...
func NewUnauthenticatedTestServer(t *testing.T, db *sql.DB) *TestServer {
	...
	// Create query runner backed by the transaction
	runner := dbrunner.NewQueryRunner(tx)

	// Create mux with transaction-backed querier, runner, and discarded logger
	// The txWrapper adds a no-op Ping() method to satisfy PingableQuerier
	mux := NewMux(&txWrapper{tx}, runner, testLogger())
