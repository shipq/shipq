...
func TestGenerateHTTPTestHarness_MuxCreation(t *testing.T) {
	...
	// Should create mux with wrapped transaction and logger
	if !strings.Contains(codeStr, "NewMux(&txWrapper{tx}, runner, testLogger())") {
		t.Error("missing NewMux call with txWrapper, runner and logger")
	}
...
// TestGenerateHTTPTestHarness_TxWrapper verifies that the generated test harness
// includes a txWrapper type to implement PingableQuerier for *sql.Tx.
// NewMux requires PingableQuerier (Querier + Ping), but *sql.Tx only implements Querier.
func TestGenerateHTTPTestHarness_TxWrapper(t *testing.T) {
	cfg := HTTPTestHarnessGenConfig{
		ModulePath: "example.com/app",
		OutputPkg:  "api",
		DBDialect:  "mysql",
	...
	// Should have txWrapper struct that embeds *sql.Tx
	if !strings.Contains(codeStr, "type txWrapper struct") {
		t.Error("TX_WRAPPER BUG: missing txWrapper struct definition")
	}

	// Should have Ping() method on txWrapper
	if !strings.Contains(codeStr, "func (w *txWrapper) Ping() error") {
		t.Error("TX_WRAPPER BUG: missing Ping() method on txWrapper")
	}

	// Should use &txWrapper{tx} instead of tx when calling NewMux
	if !strings.Contains(codeStr, "NewMux(&txWrapper{tx}") {
		t.Error("TX_WRAPPER BUG: NewMux should be called with &txWrapper{tx}, not tx")
	}

	// Should NOT have the old broken call: NewMux(tx,
	if strings.Contains(codeStr, "NewMux(tx,") {
		t.Error("TX_WRAPPER BUG: NewMux is still being called with tx (not txWrapper)")
	}
