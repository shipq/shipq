...
// When the resource requires authentication, it also generates a test verifying 401 for unauthenticated access.
func GenerateResourceTest(cfg ResourceTestGenConfig, resource ResourceInfo) ([]byte, error) {
	...
	// Generate unauthenticated 401 test if resource requires auth
	if resource.RequireAuth {
		generateUnauthenticatedTest(&buf, cfg, resource)
	}
...
func generateResourceTestImports(buf *bytes.Buffer, cfg ResourceTestGenConfig, resource ResourceInfo) {
	...
	// Config package import (for ParseDatabaseURL and IsLocalhostURL)
	...
	// Nanoid for unique email generation in auth setup
	if resource.RequireAuth {
...
func generateCRUDTestFunction(buf *bytes.Buffer, cfg ResourceTestGenConfig, resource ResourceInfo) {
	...
	// Database setup — prefer TEST_DATABASE_URL, fall back to compile-time constant from shipq.ini
	buf.WriteString("\t// Get test database connection\n")
	buf.WriteString("\tdbURL := os.Getenv(\"TEST_DATABASE_URL\")\n")
	buf.WriteString("\tif dbURL == \"\" {\n")
	fmt.Fprintf(buf, "\t\tdbURL = %q\n", cfg.TestDatabaseURL)
	buf.WriteString("\t}\n")
	buf.WriteString("\tif dbURL == \"\" {\n")
	buf.WriteString("\t\tt.Skip(\"TEST_DATABASE_URL not set, skipping integration test\")\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\tif !config.IsLocalhostURL(dbURL) {\n")
	buf.WriteString("\t\tt.Fatal(\"test database URL must point to localhost\")\n")
	buf.WriteString("\t}\n\n")
	...
	if resource.RequireAuth {
		// For auth-protected resources, sign up a user and use an authenticated client
		generateAuthSetup(buf, cfg)
	} else {
...
// generateAuthSetup generates the signup + authenticated client setup for auth-protected tests.
func generateAuthSetup(buf *bytes.Buffer, cfg ResourceTestGenConfig) {
	buf.WriteString("\t// Sign up a test user to get an authenticated session\n")
	buf.WriteString("\ttestEmail := \"test-\" + nanoid.New() + \"@example.com\"\n")
	buf.WriteString("\tsignupBody := fmt.Sprintf(`{\"email\":\"%s\",\"password\":\"testpass123\",\"first_name\":\"Test\",\"last_name\":\"User\"}`, testEmail)\n")
	buf.WriteString("\tresp, err := http.Post(ts.Server.URL+\"/signup\", \"application/json\", bytes.NewBufferString(signupBody))\n")
	buf.WriteString("\tif err != nil {\n")
	buf.WriteString("\t\tt.Fatalf(\"signup request failed: %v\", err)\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\tdefer resp.Body.Close()\n\n")

	buf.WriteString("\tif resp.StatusCode != http.StatusCreated {\n")
	buf.WriteString("\t\tt.Fatalf(\"signup failed with status %d\", resp.StatusCode)\n")
	buf.WriteString("\t}\n\n")

	buf.WriteString("\t// Extract session cookie from signup response\n")
	buf.WriteString("\tvar sessionCookie string\n")
	buf.WriteString("\tfor _, c := range resp.Cookies() {\n")
	buf.WriteString("\t\tif c.Name == \"session\" {\n")
	buf.WriteString("\t\t\tsessionCookie = c.Value\n")
	buf.WriteString("\t\t\tbreak\n")
	buf.WriteString("\t\t}\n")
	buf.WriteString("\t}\n")
	buf.WriteString("\tif sessionCookie == \"\" {\n")
	buf.WriteString("\t\tt.Fatal(\"no session cookie returned from signup\")\n")
	buf.WriteString("\t}\n\n")

	fmt.Fprintf(buf, "\tclient := %s.NewAuthenticatedTestClient(ts.Server, sessionCookie)\n\n", cfg.OutputPkg)
}

// generateUnauthenticatedTest generates a test that verifies unauthenticated access returns 401.
func generateUnauthenticatedTest(buf *bytes.Buffer, cfg ResourceTestGenConfig, resource ResourceInfo) {
	resourceName := dbstrings.ToPascalCase(resource.PackageName)
	singularName := dbstrings.ToSingular(resourceName)

	fmt.Fprintf(buf, "\nfunc TestResource_%s_Unauthenticated(t *testing.T) {\n", singularName)
	...
	// Create unauthenticated test server and client
	fmt.Fprintf(buf, "\tts := %s.NewUnauthenticatedTestServer(t, db)\n", cfg.OutputPkg)
	fmt.Fprintf(buf, "\tclient := %s.NewUnauthenticatedTestClient(ts.Server)\n\n", cfg.OutputPkg)

	buf.WriteString("\t// All CRUD operations should fail with 401 when unauthenticated\n\n")

	// Test Create returns 401
	if resource.CreateHandler != nil && resource.CreateHandler.Request != nil {
		buf.WriteString("\t// Create should fail\n")
		fmt.Fprintf(buf, "\t_, createErr := client.%s(ctx, %s.%s{})\n",
			resource.CreateHandler.FuncName, resource.PackageName, resource.CreateHandler.Request.Name)
		buf.WriteString("\tif createErr == nil {\n")
		buf.WriteString("\t\tt.Error(\"Expected 401 for unauthenticated Create, got success\")\n")
		buf.WriteString("\t}\n\n")
	}

	// Test List returns 401
	if resource.ListHandler != nil && resource.ListHandler.Request != nil {
		buf.WriteString("\t// List should fail\n")
		fmt.Fprintf(buf, "\t_, listErr := client.%s(ctx, %s.%s{})\n",
			resource.ListHandler.FuncName, resource.PackageName, resource.ListHandler.Request.Name)
		buf.WriteString("\tif listErr == nil {\n")
		buf.WriteString("\t\tt.Error(\"Expected 401 for unauthenticated List, got success\")\n")
		buf.WriteString("\t}\n\n")
	}

	// Test GetOne returns 401
	if resource.GetOneHandler != nil && resource.GetOneHandler.Request != nil {
		idField := GetResourceIDField(resource)
		buf.WriteString("\t// GetOne should fail\n")
		fmt.Fprintf(buf, "\t_, getErr := client.%s(ctx, %s.%s{%s: \"nonexistent\"})\n",
			resource.GetOneHandler.FuncName, resource.PackageName, resource.GetOneHandler.Request.Name, idField)
		buf.WriteString("\tif getErr == nil {\n")
		buf.WriteString("\t\tt.Error(\"Expected 401 for unauthenticated GetOne, got success\")\n")
		buf.WriteString("\t}\n\n")
	}

	// Test Update returns 401
	if resource.UpdateHandler != nil && resource.UpdateHandler.Request != nil {
		idField := GetResourceIDField(resource)
		buf.WriteString("\t// Update should fail\n")
		fmt.Fprintf(buf, "\t_, updateErr := client.%s(ctx, %s.%s{%s: \"nonexistent\"})\n",
			resource.UpdateHandler.FuncName, resource.PackageName, resource.UpdateHandler.Request.Name, idField)
		buf.WriteString("\tif updateErr == nil {\n")
		buf.WriteString("\t\tt.Error(\"Expected 401 for unauthenticated Update, got success\")\n")
		buf.WriteString("\t}\n\n")
	}

	// Test Delete returns 401
	if resource.DeleteHandler != nil && resource.DeleteHandler.Request != nil {
		idField := GetResourceIDField(resource)
		buf.WriteString("\t// Delete should fail\n")
		fmt.Fprintf(buf, "\tdeleteErr := client.%s(ctx, %s.%s{%s: \"nonexistent\"})\n",
			resource.DeleteHandler.FuncName, resource.PackageName, resource.DeleteHandler.Request.Name, idField)
		buf.WriteString("\tif deleteErr == nil {\n")
		buf.WriteString("\t\tt.Error(\"Expected 401 for unauthenticated Delete, got success\")\n")
		buf.WriteString("\t}\n")
	}
