...
func TestGenerateResourceTest_DatabaseSetup(t *testing.T) {
	...
	// Should get TEST_DATABASE_URL from environment (not DATABASE_URL)
	...
	// Should embed the test database URL as fallback
	if !strings.Contains(codeStr, "postgres://postgres@localhost:5432/myapp_test") {
		t.Error("missing embedded test database URL fallback")
	}

	// Should skip if TEST_DATABASE_URL not set
	if !strings.Contains(codeStr, "t.Skip") {
		t.Error("missing t.Skip for missing TEST_DATABASE_URL")
	}
	...
	// Should import only the postgres driver (pgx), not all three
	if !strings.Contains(codeStr, `"github.com/jackc/pgx/v5/stdlib"`) {
		t.Error("missing pgx driver import for postgres dialect")
	}
	...
	if strings.Contains(codeStr, `"github.com/go-sql-driver/mysql"`) {
		t.Error("should not import mysql driver for postgres dialect")
	}
...
func TestGenerateResourceTest_DifferentPackageNames(t *testing.T) {
	cfg := ResourceTestGenConfig{
		...
		OutputPkg:       "server",
		Dialect:         "mysql",
		TestDatabaseURL: "mysql://root@localhost:3306/myapp_test",
	}
	...
	if !strings.Contains(codeStr, "package spec") {
		t.Error("missing spec package declaration")
	}
