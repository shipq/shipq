...
// If a file lacks this header, shipq refuses to overwrite it so that
// user-modified files are never silently clobbered.
const GeneratedHeader = "// Code generated by shipq. DO NOT EDIT."
...
// WriteGeneratedFile safely writes a shipq-generated file. It behaves like
// WriteFileIfChanged but additionally refuses to overwrite a file that has
// been modified by the user (detected by the absence of GeneratedHeader).
//
// Rules:
//  1. File does not exist -> write it.
//  2. File exists and starts with GeneratedHeader -> overwrite if content changed.
//  3. File exists but does NOT start with GeneratedHeader -> return an error.
func WriteGeneratedFile(path string, content []byte) (bool, error) {
	...
	if !hasGeneratedHeader(existing) {
		return false, fmt.Errorf(
			"refusing to overwrite user-modified file %s — delete it or restore the %q header to allow regeneration",
			path, GeneratedHeader,
		)
	}
...
// hasGeneratedHeader reports whether data begins with the GeneratedHeader line.
func hasGeneratedHeader(data []byte) bool {
	s := string(data)
	firstLine, _, _ := strings.Cut(s, "\n")
	return strings.TrimSpace(firstLine) == GeneratedHeader
}
