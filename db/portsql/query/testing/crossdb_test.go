...
// =============================================================================
// Phase 5A-C: JSON Aggregation Typed Struct Property Tests
// =============================================================================

func TestCrossDB_JSONAggregation_TypedStruct(t *testing.T) {
	dbs, cleanup := SetupTestDBs(t)
	if dbs == nil {
		return
	}
	...
	proptest.Check(t, "JSON aggregation typed struct deserialization works identically across databases", proptest.Config{NumTrials: 20, Verbose: true}, func(g *proptest.Generator) bool {
		...
		// Build query with multi-column JSONAgg
		authorNameCol := query.StringColumn{Table: "test_authors", Name: "name"}
		authorIDCol := query.Int64Column{Table: "test_authors", Name: "id"}
		authorPublicIDCol := query.StringColumn{Table: "test_authors", Name: "public_id"}
		bookAuthorIDCol := query.Int64Column{Table: "test_books", Name: "author_id"}
		bookTitleCol := query.StringColumn{Table: "test_books", Name: "title"}
		bookPriceCol := query.NullStringColumn{Table: "test_books", Name: "price"}

		ast := query.From(MockTable("test_authors")).
			Select(authorNameCol).
			SelectJSONAgg("books", bookTitleCol, bookPriceCol).
			LeftJoin(MockTable("test_books")).On(authorIDCol.Eq(bookAuthorIDCol)).
			Where(authorPublicIDCol.Eq(query.Param[string]("public_id"))).
			GroupBy(authorNameCol).
			Build()
		...
		for _, dialect := range AllDialects() {
			...
			// Unmarshal into typed struct (same pattern the codegen will use)
			var books []BookItem
			if err := json.Unmarshal([]byte(booksRaw), &books); err != nil {
		...
		// Count should match across all dialects
		if len(pg) != len(my) || len(my) != len(sq) {
			t.Logf("book count mismatch: pg=%d my=%d sq=%d", len(pg), len(my), len(sq))
			return false
		}

		// Count should match expected
		if len(pg) != numBooks {
			t.Logf("unexpected book count: got=%d want=%d", len(pg), numBooks)
			return false
		}
...
func TestCrossDB_JSONAggregation_EmptyResult(t *testing.T) {
	dbs, cleanup := SetupTestDBs(t)
	if dbs == nil {
		return
	}
	...
	proptest.Check(t, "JSON aggregation with zero related rows returns empty slice across all databases", proptest.Config{NumTrials: 10, Verbose: true}, func(g *proptest.Generator) bool {
		dbs.ClearAllData(t)
		...
		// Query with JSON aggregation (LEFT JOIN produces no book matches)
		authorNameCol := query.StringColumn{Table: "test_authors", Name: "name"}
		authorIDCol := query.Int64Column{Table: "test_authors", Name: "id"}
		authorPublicIDCol := query.StringColumn{Table: "test_authors", Name: "public_id"}
		bookAuthorIDCol := query.Int64Column{Table: "test_books", Name: "author_id"}
		...
		for _, dialect := range AllDialects() {
			...
			// Filter out null entries (LEFT JOIN null rows)
			var filtered []BookItem
			for _, b := range books {
			...
			if len(filtered) != 0 {
				t.Logf("%s: expected 0 books for author with no books, got %d", dialect, len(filtered))
				return false
			}
		}
...
func TestCrossDB_JSONAggregation_NullableColumns(t *testing.T) {
	dbs, cleanup := SetupTestDBs(t)
	if dbs == nil {
		return
	}
	...
	proptest.Check(t, "JSON aggregation nullable columns deserialize correctly across all databases", proptest.Config{NumTrials: 15, Verbose: true}, func(g *proptest.Generator) bool {
		...
		// Insert books with some having NULL price
		numBooks := g.IntRange(2, 5)
		numWithPrice := 0
		numWithoutPrice := 0
		for i := 0; i < numBooks; i++ {
			bookPublicID := uniqueID(g.Identifier(15))
			...
			var price *string
			if g.Bool() {
				p := fmt.Sprintf("%.2f", g.Float64Range(1.0, 100.0))
				price = &p
				numWithPrice++
			} else {
				numWithoutPrice++
		...
		// Query with JSON aggregation including nullable column
		authorNameCol := query.StringColumn{Table: "test_authors", Name: "name"}
		authorIDCol := query.Int64Column{Table: "test_authors", Name: "id"}
		authorPublicIDCol := query.StringColumn{Table: "test_authors", Name: "public_id"}
		bookAuthorIDCol := query.Int64Column{Table: "test_books", Name: "author_id"}
		...
		nullCounts := make(map[Dialect]int)
		nonNullCounts := make(map[Dialect]int)
		...
		for _, dialect := range AllDialects() {
			...
			// Unmarshal into typed struct with nullable Price
			var books []BookItem
			if err := json.Unmarshal([]byte(booksRaw), &books); err != nil {
				t.Logf("JSON unmarshal error for %s: %v (json=%s)", dialect, err, booksRaw)
				return false
		...
		// Null/non-null counts should be consistent across all dialects
		pg := nullCounts[DialectPostgres]
		my := nullCounts[DialectMySQL]
		sq := nullCounts[DialectSQLite]

		if pg != my || my != sq {
			t.Logf("null price count mismatch: pg=%d my=%d sq=%d", pg, my, sq)
			return false
		}

		pgNon := nonNullCounts[DialectPostgres]
		myNon := nonNullCounts[DialectMySQL]
		sqNon := nonNullCounts[DialectSQLite]

		if pgNon != myNon || myNon != sqNon {
			t.Logf("non-null price count mismatch: pg=%d my=%d sq=%d", pgNon, myNon, sqNon)
			return false
		}

		// Counts should match expected
		if pg != numWithoutPrice {
			t.Logf("null count mismatch: got=%d expected=%d", pg, numWithoutPrice)
			return false
		}
		if pgNon != numWithPrice {
			t.Logf("non-null count mismatch: got=%d expected=%d", pgNon, numWithPrice)
			return false
		}
