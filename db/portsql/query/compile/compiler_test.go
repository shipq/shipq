...
func testExistsInSelect(t *testing.T, dialect Dialect) {
	...
	innerAST := &query.AST{
		...
		SelectCols: []query.SelectExpr{{Expr: query.ColumnExpr{Column: innerCol}}},
		Where: query.BinaryExpr{
			Left:  query.ColumnExpr{Column: innerCol},
			Op:    query.OpEq,
			Right: query.ColumnExpr{Column: query.Int64Column{Table: "users", Name: "id"}},
		},
	}
	...
	ast := &query.AST{
		...
		SelectCols: []query.SelectExpr{
			{
				Expr:  query.ExistsExpr{Subquery: innerAST},
				Alias: "has_orders",
			},
		},
	}
	...
	if !strings.Contains(sql, "EXISTS") {
		t.Errorf("SQL should contain 'EXISTS': %s", sql)
	}
	if !strings.Contains(sql, "SELECT") {
		t.Errorf("SQL should contain 'SELECT': %s", sql)
	}
	if !strings.Contains(sql, "has_orders") {
		t.Errorf("SQL should contain alias 'has_orders': %s", sql)
	}
	if !strings.Contains(sql, "orders") {
		t.Errorf("SQL should contain 'orders' from subquery: %s", sql)
	}
