...
func generateRandomExpr(gen *proptest.Generator, maxDepth int) Expr {
	...
	exprTypes := []int{0, 1, 2, 3, 4, 5, 6, 7, 8} // column, param, literal, binary, unary, func, aggregate, exists, subquery
	...
	case 7: // Exists
		return ExistsExpr{
			Subquery: generateRandomSubqueryAST(gen),
			Negated:  gen.Bool(),
		}
	case 8: // Subquery
		return SubqueryExpr{
			Query: generateRandomSubqueryAST(gen),
		}
	default:
		return generateLeafExpr(gen)
	}
...
// generateRandomSubqueryAST generates a simple SELECT AST suitable for use in
// EXISTS or subquery expressions. Uses only leaf expressions to avoid deep recursion.
func generateRandomSubqueryAST(gen *proptest.Generator) *AST {
	tableName := gen.Identifier(20)
	numCols := gen.IntRange(1, 3)
	selectCols := make([]SelectExpr, numCols)
	for i := 0; i < numCols; i++ {
		selectCols[i] = SelectExpr{
			Expr: ColumnExpr{Column: StringColumn{Table: tableName, Name: gen.Identifier(20)}},
		}
	}
	return &AST{
		Kind:       SelectQuery,
		FromTable:  TableRef{Name: tableName},
		SelectCols: selectCols,
		Where:      generateLeafExpr(gen),
	}
}
