...
type TableAnalysis struct {
	...
	HasUpdatedAt       bool
	HasDeletedAt       bool
	HasAuthorAccountID bool // True if table has author_account_id (auto-populated from session)
...
func AnalyzeTable(table ddl.Table) TableAnalysis {
	...
	for _, col := range table.Columns {
		...
		case "author_account_id":
			analysis.HasAuthorAccountID = true
		}
	...
	// author_account_id is auto-populated from the authenticated session.
	for _, col := range table.Columns {
		if col.Name == "id" || col.Name == "public_id" ||
			col.Name == "created_at" || col.Name == "updated_at" ||
			col.Name == "deleted_at" || col.Name == "author_account_id" {
			continue
		}
		analysis.UserColumns = append(analysis.UserColumns, col)
	}

	// ResultColumns: exclude internal id, deleted_at, and author_account_id
	// (author_account_id is an internal FK, not exposed in API responses)
	for _, col := range table.Columns {
		if col.Name == "id" || col.Name == "deleted_at" || col.Name == "author_account_id" {
			continue
		}
...
type TypeMapping struct {
	...
	SQLiteScanType string // intermediate scan type for SQLite, empty means direct scan
}
...
// NeedsSQLiteScanIntermediate reports whether a column should scan into an
// intermediate SQLite type before conversion to its final Go type.
func NeedsSQLiteScanIntermediate(col ddl.ColumnDefinition) bool {
	return MapColumnType(col).SQLiteScanType != ""
}
...
func GenerateTableStruct(table ddl.Table, queryPkgPath string) ([]byte, error) {
	...
	buf.WriteString("// TableName returns the SQL table name.\n")
