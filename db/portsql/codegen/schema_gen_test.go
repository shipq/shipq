...
func TestAnalyzeTable_WithAuthorAccountID(t *testing.T) {
	table := ddl.Table{
		Name: "posts",
		Columns: []ddl.ColumnDefinition{
			{Name: "id", Type: ddl.BigintType, PrimaryKey: true},
	...
	if !analysis.HasAuthorAccountID {
		t.Error("expected HasAuthorAccountID = true")
	}

	// UserColumns should NOT include author_account_id
	for _, col := range analysis.UserColumns {
		if col.Name == "author_account_id" {
			t.Error("UserColumns should NOT contain author_account_id")
		}
	}

	// UserColumns should only have title and body
	if len(analysis.UserColumns) != 2 {
		t.Errorf("expected 2 UserColumns, got %d", len(analysis.UserColumns))
		for _, col := range analysis.UserColumns {
			t.Logf("  UserColumn: %s", col.Name)
		}
	}

	// ResultColumns should NOT include author_account_id
	for _, col := range analysis.ResultColumns {
		if col.Name == "author_account_id" {
			t.Error("ResultColumns should NOT contain author_account_id")
		}
	}
}

func TestAnalyzeTable_WithoutAuthorAccountID(t *testing.T) {
	table := ddl.Table{
		Name: "posts",
		Columns: []ddl.ColumnDefinition{
			{Name: "id", Type: ddl.BigintType, PrimaryKey: true},
	...
	if analysis.HasAuthorAccountID {
		t.Error("expected HasAuthorAccountID = false")
	}
}
...
func TestMapColumnType(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if mapping.SQLiteScanType != tt.wantSQLiteScan {
				t.Errorf("SQLiteScanType = %q, want %q", mapping.SQLiteScanType, tt.wantSQLiteScan)
			}
		})
	}
}

func TestNeedsSQLiteScanIntermediate(t *testing.T) {
	cases := []struct {
		col  ddl.ColumnDefinition
		want bool
	}{
		{col: ddl.ColumnDefinition{Type: ddl.StringType}, want: false},
		{col: ddl.ColumnDefinition{Type: ddl.BigintType}, want: false},
		{col: ddl.ColumnDefinition{Type: ddl.DatetimeType}, want: true},
		{col: ddl.ColumnDefinition{Type: ddl.TimestampType, Nullable: true}, want: true},
		{col: ddl.ColumnDefinition{Type: ddl.JSONType}, want: true},
	}

	for _, c := range cases {
		got := NeedsSQLiteScanIntermediate(c.col)
		if got != c.want {
			t.Fatalf("NeedsSQLiteScanIntermediate(%v) = %v, want %v", c.col.Type, got, c.want)
		}
	}
}

func TestProperty_SQLiteScanMapping_DatetimeAlwaysIntermediate(t *testing.T) {
	proptest.QuickCheck(t, "datetime columns require sqlite intermediate scan", func(g *proptest.Generator) bool {
		colType := proptest.OneOf(g, ddl.DatetimeType, ddl.TimestampType)
		col := ddl.ColumnDefinition{Type: colType, Nullable: g.Bool()}
		return NeedsSQLiteScanIntermediate(col) && MapColumnType(col).SQLiteScanType != ""
	})
}

func TestProperty_SQLiteScanMapping_JSONAlwaysIntermediate(t *testing.T) {
	proptest.QuickCheck(t, "json columns require sqlite intermediate scan", func(g *proptest.Generator) bool {
		col := ddl.ColumnDefinition{Type: ddl.JSONType, Nullable: g.Bool()}
		m := MapColumnType(col)
		return NeedsSQLiteScanIntermediate(col) && (m.SQLiteScanType == "string" || m.SQLiteScanType == "sql.NullString")
	})
}

func TestProperty_SQLiteScanMapping_PrimitivesDirectScan(t *testing.T) {
	proptest.QuickCheck(t, "primitive columns do not require sqlite intermediate scan", func(g *proptest.Generator) bool {
		primitive := proptest.OneOf(g, ddl.IntegerType, ddl.BigintType, ddl.DecimalType, ddl.FloatType, ddl.BooleanType, ddl.StringType, ddl.TextType, ddl.BinaryType)
		col := ddl.ColumnDefinition{Type: primitive, Nullable: g.Bool()}
		return !NeedsSQLiteScanIntermediate(col) && MapColumnType(col).SQLiteScanType == ""
	})
}
