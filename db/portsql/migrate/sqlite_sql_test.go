...
func TestSQLite_CreateTable_JunctionTable_HasAutoincrement(t *testing.T) {
	...
	// Junction tables use AddTable with standard resource columns and SHOULD get INTEGER PRIMARY KEY (rowid semantics)
	if !strings.Contains(sql, `"id" INTEGER PRIMARY KEY`) {
		t.Errorf("junction table should have INTEGER PRIMARY KEY for id column, got:\n%s", sql)
	}
}
...
// =============================================================================
// MakeTable Timestamp Default Tests
// =============================================================================

// TestSQLite_CreateTable_MakeTable_TimestampDefaults verifies that a table
// created via MakeTable produces SQLite DDL with correct DEFAULT values for
// created_at and updated_at, and that deleted_at is nullable (no NOT NULL).
func TestSQLite_CreateTable_MakeTable_TimestampDefaults(t *testing.T) {
	tb := ddl.MakeTable("test_timestamps")
	tb.String("name")
	table := tb.Build()
	...
	// Helper: extract the column definition fragment for a given column name.
	// SQLite CREATE TABLE is a single line like:
	//   CREATE TABLE "t" ("id" INTEGER PRIMARY KEY, "created_at" TEXT NOT NULL DEFAULT ..., "deleted_at" TEXT, ...);
	// We split on ", " to get individual column defs, then find the one containing the column name.
	findColDef := func(colName string) string {
		...
		// Split on comma-space to get individual column defs
	...
	// created_at must have DEFAULT and NOT NULL
	createdAtDef := findColDef("created_at")
	if createdAtDef == "" {
		t.Fatal("expected created_at column in DDL")
	}
	if !strings.Contains(createdAtDef, "DEFAULT") {
		t.Errorf("created_at should have a DEFAULT clause, got: %s", createdAtDef)
	}
	if !strings.Contains(createdAtDef, "NOT NULL") {
		t.Errorf("created_at should be NOT NULL, got: %s", createdAtDef)
	}

	// updated_at must have DEFAULT and NOT NULL
	updatedAtDef := findColDef("updated_at")
	if updatedAtDef == "" {
		t.Fatal("expected updated_at column in DDL")
	}
	if !strings.Contains(updatedAtDef, "DEFAULT") {
		t.Errorf("updated_at should have a DEFAULT clause, got: %s", updatedAtDef)
	}
	if !strings.Contains(updatedAtDef, "NOT NULL") {
		t.Errorf("updated_at should be NOT NULL, got: %s", updatedAtDef)
	}

	// deleted_at must NOT have NOT NULL and must NOT have DEFAULT
	deletedAtDef := findColDef("deleted_at")
	if deletedAtDef == "" {
		t.Fatal("expected deleted_at column in DDL")
	}
	if strings.Contains(deletedAtDef, "NOT NULL") {
		t.Errorf("deleted_at should be nullable (no NOT NULL), got: %s", deletedAtDef)
	}
	if strings.Contains(deletedAtDef, "DEFAULT") {
		t.Errorf("deleted_at should not have a DEFAULT clause, got: %s", deletedAtDef)
	}
}
