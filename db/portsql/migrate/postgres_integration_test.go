...
func TestPostgresIntegration_AddTable_DefaultColumns(t *testing.T) {
	...
	// Check deleted_at: nullable (used for soft-delete)
	deletedAtCol := findColumn(columns, "deleted_at")
	if deletedAtCol == nil {
		t.Error("expected 'deleted_at' column to exist")
	} else {
		if deletedAtCol.DataType != "timestamp with time zone" {
			t.Errorf("expected deleted_at type 'timestamp with time zone', got '%s'", deletedAtCol.DataType)
		}
		if !deletedAtCol.IsNullable {
