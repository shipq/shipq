...
func generateSQLiteAlterTable(tableName string, ops []ddl.TableOperation, currentTable *ddl.Table) string {
	// If any operation requires a table rebuild (e.g., changing nullability,
	// column type, or default), use the rebuild approach instead of
	// per-operation ALTER TABLE statements.
	if requiresTableRebuild(ops) {
		return generateSQLiteTableRebuild(tableName, currentTable, ops)
	}
...
func generateSQLiteTableRebuild(tableName string, currentTable *ddl.Table, ops []ddl.TableOperation) string {
	...
	// Note: the migration runner already wraps each migration in a
	// transaction, so we must NOT emit BEGIN/COMMIT here (SQLite does
