...
```
Table Definition → SQL Generation → Execute → Introspect → Normalized Schema
```
...
| `RenameColumn(a→b) + RenameColumn(b→a) = identity`
...
- Unicode characters in defaults (`日本語`, `emoji 🎉`)
...
- `true` → maps to `1` in SQLite/MySQL
- `false` → maps to `0` in SQLite/MySQL
...
├── property_generators.go       # DDL generators (columns, tables, operations)
├── property_normalize.go        # Schema normalization functions
├── property_normalize_test.go   # Unit tests for normalization logic
├── property_crossdb_test.go     # Cross-database equivalence tests
├── property_roundtrip_test.go   # Roundtrip consistency tests
├── property_algebra_test.go     # Operation algebra tests
├── property_fuzz_test.go        # SQL validity fuzzing tests
├── property_constraint_test.go  # Constraint enforcement tests
----
...
import "github.com/shipq/shipq/db/proptest"
...
// Main property: same MigrationPlan → equivalent schemas
...
// Property: Definition → SQL → Execute → Introspect ≈ Definition
...
// Property: RenameColumn(a→b) + RenameColumn(b→a) = identity
...
1. **Build proptest package**: Generic property testing framework with seeded generators ✓
