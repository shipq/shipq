...
// TestProperty_JunctionTable_HasAutoincrement verifies that junction tables
// with a single integer PK get autoincrement like any other resource table.
// Junction tables now use AddTable and get all standard resource columns.
func TestProperty_JunctionTable_HasAutoincrement(t *testing.T) {
	proptest.QuickCheck(t, "junction table has autoincrement", func(g *proptest.Generator) bool {
		table := generateJunctionTable(g)

		_, ok := GetAutoincrementPK(table)
		if !ok {
			t.Log("Junction table with single integer PK should be autoincrement eligible")
			return false
		}

		pgSQL := generatePostgresCreateTable(table)
		if !strings.Contains(pgSQL, "GENERATED") && !strings.Contains(pgSQL, "IDENTITY") {
			t.Logf("Postgres SQL missing GENERATED/IDENTITY for junction table:\n%s", pgSQL)
			return false
		}

		mySQL := generateMySQLCreateTable(table)
		if !strings.Contains(mySQL, "AUTO_INCREMENT") {
			t.Logf("MySQL SQL missing AUTO_INCREMENT for junction table:\n%s", mySQL)
			return false
		}
