	...
	for _, colName := range []string{"created_at", "updated_at"} {
		...
		if col == nil {
			...
			if col.DefaultVal == nil {
				t.Errorf("expected %s to have a DEFAULT value", colName)
			}
		}
	}

	// Check deleted_at: nullable, no default
	deletedAtCol := findSQLiteColumn(columns, "deleted_at")
	if deletedAtCol == nil {
		t.Error("expected 'deleted_at' column to exist")
	} else {
		if deletedAtCol.Type != "TEXT" {
			t.Errorf("expected deleted_at type 'TEXT', got '%s'", deletedAtCol.Type)
		}
		if deletedAtCol.NotNull {
			t.Error("expected deleted_at to be nullable (NOT NULL should be false)")
		}
	}
...
// =============================================================================
// MakeTable Timestamp Defaults Integration Tests
// =============================================================================

// TestSQLiteIntegration_MakeTable_InsertWithoutTimestamps verifies that a table
// created via MakeTable (AddTable) can accept INSERTs that omit the auto-managed
// timestamp columns (created_at, updated_at, deleted_at). This is the exact
// scenario that broke when those columns were NOT NULL without DEFAULT values.
func TestSQLiteIntegration_MakeTable_InsertWithoutTimestamps(t *testing.T) {
	db := connectSQLite(t)
	defer db.Close()

	tableName := "test_timestamp_defaults"

	// Create table using AddTable (which calls MakeTable internally)
	plan := &MigrationPlan{Schema: Schema{Tables: map[string]ddl.Table{}}}
	_, err := plan.AddTable(tableName, func(tb *ddl.TableBuilder) error {
		tb.String("name")
		return nil
	...
	// INSERT providing ONLY id, public_id, and the user column.
	// Omit created_at, updated_at, deleted_at entirely -- they must be handled
	// by database defaults (or be nullable in the case of deleted_at).
	_, err = db.Exec(fmt.Sprintf(
		`INSERT INTO "%s" ("id", "public_id", "name") VALUES (1, 'abc123', 'Test User')`,
		tableName,
	))
	if err != nil {
		t.Fatalf("INSERT without timestamps failed (this is the bug we are guarding against): %v", err)
	}

	// Query back the row and verify timestamp column values
	var createdAt, updatedAt sql.NullString
	var deletedAt sql.NullString
	var name string

	err = db.QueryRow(fmt.Sprintf(
		`SELECT "name", "created_at", "updated_at", "deleted_at" FROM "%s" WHERE "id" = 1`,
		tableName,
	...
	// created_at must not be NULL (should have been filled by DEFAULT)
	if !createdAt.Valid {
		t.Error("expected created_at to be non-NULL (filled by DEFAULT), but it was NULL")
	} else {
		t.Logf("created_at = %s", createdAt.String)
	}

	// updated_at must not be NULL (should have been filled by DEFAULT)
	if !updatedAt.Valid {
		t.Error("expected updated_at to be non-NULL (filled by DEFAULT), but it was NULL")
	} else {
		t.Logf("updated_at = %s", updatedAt.String)
	}

	// deleted_at MUST be NULL (active record, column should be nullable with no default)
	if deletedAt.Valid {
		t.Errorf("expected deleted_at to be NULL for active record, but got %q", deletedAt.String)
	}
}
