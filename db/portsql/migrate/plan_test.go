...
func TestAddTableWithDefaults_IncludesDefaultColumns(t *testing.T) {
	...
	// Check created_at and updated_at: NOT NULL with DEFAULT
	for _, colName := range []string{"created_at", "updated_at"} {
		...
		if col.Default == nil || *col.Default != "CURRENT_TIMESTAMP" {
			t.Errorf("expected %s to have DEFAULT CURRENT_TIMESTAMP", colName)
		}
	}

	// Check deleted_at: nullable, no default
	deletedAtCol := columnsByName["deleted_at"]
	if deletedAtCol.Type != ddl.DatetimeType {
		t.Errorf("expected deleted_at type %q, got %q", ddl.DatetimeType, deletedAtCol.Type)
	}
	if !deletedAtCol.Nullable {
		t.Error("expected deleted_at to be nullable")
	}
	if deletedAtCol.Default != nil {
		t.Errorf("expected deleted_at to have no default, got %q", *deletedAtCol.Default)
	}
...
func TestMigrationPlan_MultipleMigrations(t *testing.T) {
	...
	if !strings.Contains(plan.Migrations[1].Name, "_alter_users") {
		t.Errorf("expected second migration to contain '_alter_users', got %q", plan.Migrations[1].Name)
	}
...
// =============================================================================
// Author Account ID Auto-Tracking Tests
// =============================================================================

// buildAuthSchema creates a plan with the standard auth tables (accounts, organizations,
// organization_users, sessions), simulating what `shipq auth` produces.
func buildAuthSchema(t *testing.T) *MigrationPlan {
	t.Helper()
	plan := NewPlan()

	// Create organizations first
	plan.SetCurrentMigration("20260101000000_create_organizations")
	...
	if err != nil {
		t.Fatalf("AddTable organizations failed: %v", err)
	}

	orgRef, _ := plan.Table("organizations")

	// Create accounts
	plan.SetCurrentMigration("20260101000001_create_accounts")
	_, err = plan.AddTable("accounts", func(tb *ddl.TableBuilder) error {
		tb.String("first_name")
		tb.String("last_name")
		tb.String("email").Unique()
		tb.Binary("password_hash")
		tb.Bigint("default_organization_id").References(orgRef).Nullable()
		return nil
	})
	if err != nil {
		t.Fatalf("AddTable accounts failed: %v", err)
	}

	accountsRef, _ := plan.Table("accounts")

	// Create organization_users (junction — still gets standard resource columns)
	plan.SetCurrentMigration("20260101000002_create_organization_users")
	_, err = plan.AddTable("organization_users", func(tb *ddl.TableBuilder) error {
		orgIDCol := tb.Bigint("organization_id").References(orgRef).Col()
	...
	if err != nil {
		t.Fatalf("AddTable organization_users failed: %v", err)
	}

	// Create sessions
	plan.SetCurrentMigration("20260101000003_create_sessions")
	...
	if err != nil {
		t.Fatalf("AddTable sessions failed: %v", err)
	}

	return plan
}

func TestAddTable_AutoAddsAuthorAccountID_WhenAuthExists(t *testing.T) {
	plan := buildAuthSchema(t)

	// Now create a user-defined table - should get author_account_id
	plan.SetCurrentMigration("20260102000000_create_posts")
	...
	if err != nil {
		t.Fatalf("AddTable posts failed: %v", err)
	}

	postsTable := plan.Schema.Tables["posts"]

	// Find author_account_id column
	found := false
	for _, col := range postsTable.Columns {
		if col.Name == "author_account_id" {
			found = true
			if col.Type != ddl.BigintType {
				t.Errorf("expected author_account_id type %q, got %q", ddl.BigintType, col.Type)
			}
			if col.References != "accounts" {
				t.Errorf("expected author_account_id to reference 'accounts', got %q", col.References)
			}
			if col.Nullable {
				t.Error("expected author_account_id to NOT be nullable")
			}
			break
		}
	}
	if !found {
		t.Error("expected author_account_id column to be auto-added to posts table")
	}
}

func TestAddTable_DoesNotAddAuthorAccountID_WhenNoAuth(t *testing.T) {
	plan := NewPlan()

	// Create a table without auth schema
	plan.SetCurrentMigration("20260101000000_create_posts")
	_, err := plan.AddTable("posts", func(tb *ddl.TableBuilder) error {
		tb.String("title")
		tb.Text("body")
		return nil
	...
	for _, col := range postsTable.Columns {
		if col.Name == "author_account_id" {
			t.Error("author_account_id should NOT be added when accounts table doesn't exist")
		}
	}
}

func TestAddTable_DoesNotAddAuthorAccountID_ToAuthTables(t *testing.T) {
	// Verify auth tables don't get author_account_id
	plan := buildAuthSchema(t)

	for _, tableName := range []string{"accounts", "organizations", "sessions"} {
		table := plan.Schema.Tables[tableName]
		for _, col := range table.Columns {
			if col.Name == "author_account_id" {
				t.Errorf("auth table %q should NOT have author_account_id", tableName)
			}
		}
	}
}

func TestAddTable_AutoAddsAuthorAccountID_IncludesInSQL(t *testing.T) {
	plan := buildAuthSchema(t)

	plan.SetCurrentMigration("20260102000000_create_articles")
	_, err := plan.AddTable("articles", func(tb *ddl.TableBuilder) error {
		tb.String("title")
		return nil
	})
	if err != nil {
		t.Fatalf("AddTable articles failed: %v", err)
	}

	// Find the articles migration
	var articlesMigration *Migration
	for i := range plan.Migrations {
		if plan.Migrations[i].Name == "20260102000000_create_articles" {
			articlesMigration = &plan.Migrations[i]
			break
		}
	}
	if articlesMigration == nil {
		t.Fatal("could not find articles migration")
	}

	// Verify all SQL dialects include author_account_id
	if !strings.Contains(articlesMigration.Instructions.Postgres, "author_account_id") {
		t.Error("expected Postgres SQL to include author_account_id")
	}
	if !strings.Contains(articlesMigration.Instructions.MySQL, "author_account_id") {
		t.Error("expected MySQL SQL to include author_account_id")
	}
	if !strings.Contains(articlesMigration.Instructions.Sqlite, "author_account_id") {
		t.Error("expected SQLite SQL to include author_account_id")
	}
}

func TestAddTable_MultipleUserTables_AllGetAuthorAccountID(t *testing.T) {
	plan := buildAuthSchema(t)

	tables := []string{"posts", "comments", "tags"}
	for i, name := range tables {
		plan.SetCurrentMigration(strings.Replace("20260102XXXXX_create_", "XXXXX", strings.Repeat("0", 5-len(string(rune('0'+i)))), 1) + name)
		_, err := plan.AddTable(name, func(tb *ddl.TableBuilder) error {
			tb.String("content")
			return nil
		})
		if err != nil {
			t.Fatalf("AddTable %s failed: %v", name, err)
		}
	}
	...
	for _, name := range tables {
		...
		if !found {
			t.Errorf("table %q should have author_account_id", name)
		}
	}
}
...
func TestAddTable_DoesNotAddAuthorAccountID_ToJunctionTables(t *testing.T) {
	...
	// the junction table validation.
	plan := buildAuthSchema(t)

	// Create a regular table first (should still get author_account_id)
	plan.SetCurrentMigration("20260102000000_create_managed_files")
	...
	if err != nil {
		t.Fatalf("AddTable managed_files failed: %v", err)
	}

	managedFilesRef, err := plan.Table("managed_files")
	if err != nil {
		t.Fatalf("could not get managed_files ref: %v", err)
	}

	accountsRef, err := plan.Table("accounts")
	if err != nil {
		t.Fatalf("could not get accounts ref: %v", err)
	}

	// Create a junction table referencing managed_files and accounts
	plan.SetCurrentMigration("20260102000001_create_file_access")
	_, err = plan.AddTable("file_access", func(tb *ddl.TableBuilder) error {
		fileCol := tb.Bigint("managed_file_id").References(managedFilesRef).Col()
		acctCol := tb.Bigint("account_id").References(accountsRef).Col()
		tb.String("role")
		tb.AddUniqueIndex(fileCol, acctCol)
		tb.JunctionTable()
		return nil
	})
	if err != nil {
		t.Fatalf("AddTable file_access failed: %v", err)
	}

	fileAccessTable := plan.Schema.Tables["file_access"]

	// Verify author_account_id was NOT added
	for _, col := range fileAccessTable.Columns {
		if col.Name == "author_account_id" {
			t.Error("junction table file_access should NOT have author_account_id auto-added")
		}
	}

	// Verify it still has exactly 2 References columns
	refCount := 0
	for _, col := range fileAccessTable.Columns {
		if col.References != "" {
			refCount++
	...
	if refCount != 2 {
		t.Errorf("expected junction table to have exactly 2 References columns, got %d", refCount)
	}
}
