...
func TestMySQLIntegration_AddTable_DefaultColumns(t *testing.T) {
	...
	// Check deleted_at: nullable (soft-delete sentinel)
	deletedAtCol := findMySQLColumn(columns, "deleted_at")
	if deletedAtCol == nil {
		t.Error("expected 'deleted_at' column to exist")
	} else {
		if deletedAtCol.DataType != "datetime" {
			t.Errorf("expected deleted_at type 'datetime', got '%s'", deletedAtCol.DataType)
		}
