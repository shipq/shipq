...
func (c *SQLiteCompiler) compileSelect(ast *query.AST) (string, error) {
	var b strings.Builder
...
func (c *SQLiteCompiler) compileUpdate(ast *query.AST) (string, error) {
	var b strings.Builder
...
func (c *SQLiteCompiler) compileDelete(ast *query.AST) (string, error) {
	var b strings.Builder
...
func (c *SQLiteCompiler) writeExpr(b *strings.Builder, expr query.Expr) error {
	switch e := expr.(type) {
	case query.ColumnExpr:
		c.writeColumn(b, e.Column)

	case query.ParamExpr:
		// SQLite uses ? placeholder (like MySQL)
...
func (c *SQLiteCompiler) writeIdentifier(b *strings.Builder, name string) {
	// SQLite uses double quotes for identifiers (like Postgres)
	fmt.Fprintf(b, `"%s"`, name)
}

func (c *SQLiteCompiler) writeColumn(b *strings.Builder, col query.Column) {
...
func (c *SQLiteCompiler) writeLiteral(b *strings.Builder, val any) {
	...
	case bool:
		// SQLite uses 1/0 for booleans (like MySQL)
		if v {
			b.WriteString("1")
		} else {
			b.WriteString("0")
...
func (c *SQLiteCompiler) writeCTEs(b *strings.Builder, ctes []query.CTE) error {
	b.WriteString("WITH ")
...
func (c *SQLiteCompiler) compileSetOp(ast *query.AST) (string, error) {
	var b strings.Builder

	// Compile left query
	// Note: SQLite doesn't support parentheses around individual SELECT statements in set operations
	leftCompiler := &SQLiteCompiler{}
