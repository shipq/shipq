...
// The result is always a valid Go identifier (when the input is non-empty):
//   - Empty parts from leading/trailing/consecutive underscores are skipped.
//   - If the result would start with a digit, it is prefixed with "X".
//   - If the input is non-empty but produces no alphanumeric content (e.g. "_"),
//     "X" is returned as a fallback.
//
// Examples:
//
//	"user_id" -> "UserId"
...
func ToPascalCase(s string) string {
	...
	if out == "" {
		if s != "" {
			return "X" // non-empty input produced no identifier chars
		}
		return ""
	}
	// Go identifiers cannot start with a digit. Prefix with "X" if needed.
	if out[0] >= '0' && out[0] <= '9' {
		return "X" + out
	}
	return out
