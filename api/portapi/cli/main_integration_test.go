...
`
	...
	// 2. Create shipq.ini pointing to full import path
	iniContent := `[db]
dialects = mysql
...
`
	...
	if err := runGenerator(os.Stdout, os.Stderr); err != nil {
...
func TestMain_EndToEnd_RelativePackagePath(t *testing.T) {
	// Get the module dir BEFORE changing directories
	modDir := getModuleDir(t)

	// Test that relative package paths work (e.g., ./myapi)
	tmpDir := t.TempDir()
...
`
	...
	// Create shipq.ini with relative path
	iniContent := `[db]
dialects = mysql
...
func TestMain_EndToEnd_NoConfig(t *testing.T) {
	...
	err = runGenerator(os.Stdout, os.Stderr)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	if !strings.Contains(err.Error(), "shipq.ini not found") {
		t.Errorf("expected error to contain 'shipq.ini not found', got %q", err.Error())
	}
}
...
func TestMain_EndToEnd_InvalidPackage(t *testing.T) {
	...
	// Create shipq.ini pointing to non-existent package
	iniContent := `[db]
dialects = mysql
...
`
	...
	// Run first time
	if err := runGenerator(os.Stdout, os.Stderr); err != nil {
		t.Fatalf("first runGenerator() failed: %v", err)
	}
	...
	// Run second time
	if err := runGenerator(os.Stdout, os.Stderr); err != nil {
		t.Fatalf("second runGenerator() failed: %v", err)
	}
...
func TestMain_EndToEnd_MissingPackage(t *testing.T) {
	// Test that missing [api] package produces a clear error
	tmpDir := t.TempDir()

	// Create shipq.ini without package
	iniContent := `[db]
dialects = mysql
...
`
	...
	if !strings.Contains(err.Error(), "missing [api] package") {
		t.Errorf("expected error to contain 'missing [api] package', got %q", err.Error())
	}
}
